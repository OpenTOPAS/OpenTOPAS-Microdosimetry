...
void TsYScorer::AbsorbResultsFromWorkerScorer(TsVScorer* workerScorer) {
    ...
    for (size_t i=0; i<yVector_Particle_worker.size(); i++){
...
void TsYScorer::UserHookForEndOfRun()
{
    ...
    if (fGetStatisticInfo){
        ...
        clock_t start,end;
        start = clock();
        if (fSpectrumUpdateTimes>(G4int)yVector.size()){
            G4int size = (G4int)yVector.size();
        ...
        for (G4int i=1; i<=fSpectrumUpdateTimes; i++){
            ...
            for(G4int j=0; j<dataSize*i && j<(G4int)yVector.size(); j++)
